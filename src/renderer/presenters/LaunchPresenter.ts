...
export class LaunchPresenter {
    ...
    private results: Array<SearchResult<Resource>>;
    ...
    public onInputChanged(): void {
        ...
        for (const result of this.results) {
            ...
            const app = Platform.appCache.filter((app) => app.name === result.record.appName)[0];  //.appCache[result.record.appName];
            viewItems.push({
    ...
    private getResultById(id: number): SearchResult<Resource> {
        for (const result of this.results) {
            if (result.id === id) {
                return result;
            }
