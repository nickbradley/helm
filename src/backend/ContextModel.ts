...
export class ContextModel {
  ...
  public async search(opts: {searchTerm: string, project: string}) {
    ...
    const fileResults = fileRawResults.map((r: any) => ({
      ...
      title: r.file,
      description: "",
      path: r.client === "aw-watcher-idea" ? `${this.projects[this.project].root}/${r.file}` : r.file,
      relevance: r.relevance,
      group: "file",
      custom: {
        tracker: r.client,
    ...
    // Sort the groups by their most relevant action
    const groups: any[] = [];
    if (windowResults.length >= 1) {
      groups.push({
        relevance: windowResults[0].relevance,
    ...
    if (browserResults.length >= 1) {
      groups.push({
        relevance: browserRawResults[0].relevance,
    ...
    if (fileResults.length >= 1) {
      groups.push({
        relevance: fileResults[0].relevance,
        results: fileResults
      });
    }
    ...
    if (shellResults.length >= 1) {
      groups.push({
        relevance: shellResults[0].relevance,
